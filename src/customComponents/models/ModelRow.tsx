...
export const DataParameterRow: React.FC<{ data: ConfigParameter }> = ({
  data,
}) => (
  <tr>
...
const ModelBuildConfigCompare: React.FC<any> = ({
  dataBuildConfigLeft,
  ...
  <div className="row">
    <div className="col">
      <TableSection title={"Configuration"} columns={[""]}>
        {dataBuildConfigLeft &&
          Array.isArray(dataBuildConfigLeft) &&
          dataBuildConfigLeft.map((d: ConfigParameter, index: any) => (
    ...
    <div className="col">
      <TableSection title={"Configuration"} columns={[""]}>
        {dataBuildConfigRight &&
          Array.isArray(dataBuildConfigRight) &&
          dataBuildConfigRight.map((d: ConfigParameter, index: any) => (
...
const ModelRow: React.FC<any> = ({ state }) => {
    ...
    const fetchLabels = async () => {
      try {
        const label1 = selectedModelLeft
          ? await extractLabelsFromDataset(selectedModelLeft)
          : null;
        const label2 = selectedModelRight
          ? await extractLabelsFromDataset(selectedModelRight)
          : null;
  ...
  return (
    <div className="container-fluid">
      {selectedCriteria === "Build Configuration" &&
      ...
      {selectedCriteria === "Model Performance" &&
        (cmConfigLeft || cmConfigRight) && (
      ...
      {selectedCriteria === "Confusion Matrix" &&
        (cmConfigLeft || cmConfigRight) && (
          <ConfusionMatrix
