...
const AttackBarChart: React.FC<AttackBarChartProps> = ({ dataSets, attackStatus }) => {
    ...
    const maxCount = Math.max(...data.map(d => Math.max(d.original, d.poisoned)));

    return (
        <VictoryChart domainPadding={20} theme={VictoryTheme.material} domain={{y: [0, maxCount]}}>
            <VictoryAxis
                label="Class Labels"
                style={{
            ...
            <VictoryGroup offset={10} colorScale={"qualitative"}>
                <VictoryBar
                    ...
                    labelComponent={<VictoryTooltip
                        flyoutStyle={{ fill: "white", stroke: "lightgrey", strokeWidth: 1 }}
                    />}
