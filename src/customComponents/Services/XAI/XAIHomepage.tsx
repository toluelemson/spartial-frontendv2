...
const XAIHomepage: React.FC = () => {
  ...
  const [results, setResults] = useState<{ accuracy: number } | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await clf_accuracy_metric(
        groundTruthArray,
        predictionsArray
      );
  ...
  return (
    <>
      <div>
        <XAINavbar />
        <br />
        <h2 className="text-gray">Building Trust Through Explainable AI</h2>
