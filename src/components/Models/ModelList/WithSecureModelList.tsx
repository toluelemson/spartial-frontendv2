...
import useWithSecureModelAPI from "../WithSecureModelAPI";
import { deleteMedicalModel } from "../../../api";
import { ConvertTimeStamp } from "../../util/utility";

interface WithSecureModelListProps {
  models?: ModelData[] | null;
}

const WithSecureModelList: FC<WithSecureModelListProps> = () => {
  console.log("WithSecureModelList component mounted");
  const navigate = useNavigate();
  const { models, loading, error } = useWithSecureModelAPI();
  ...
  const handleNavigate = (modelId: string) => {
    const trimmedModelId = modelId.replace(/^ws-/, "");
...
export default WithSecureModelList;
